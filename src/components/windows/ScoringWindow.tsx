...
const ROWS: { en: [string, string]; zh: [string, string] }[] = [
  ...
  { en: ['Depth of MuJoCo Use', 'How thoroughly MJCF, physics simulation, collisions, joints, sensors and actuators are used.'], zh: ['MuJoCo 使用深度', '是否充分利用 MJCF、物理仿真、碰撞、关节、传感器、执行器。'] },
  { en: ['Task Design', 'Whether the task is clear, challenging and meaningful.'], zh: ['任务设计', '是否清晰、有挑战性、有现实意义。'] },
  { en: ['Control', 'Teleoperation, autonomous control, policy control, task planning or data-collection capability.'], zh: ['控制能力', '遥操作、自动控制、策略控制、任务规划或数据采集能力。'] },
  ...
  { en: ['Engineering Quality', 'Clarity of code structure, docs, configuration and asset management.'], zh: ['工程质量', '代码结构、文档、配置与资源管理是否清晰。'] },
...
export default function ScoringWindow() {
  ...
  return (
    <Window id="w-scoring" title="scoring.cfg" bodyClassName="c ctext">
      ...
      <p>
        {t(
          'All three AI judges (Claude / GPT / Gemini) receive an identical review package (demo video + code + README / write-up). Anchored absolute scoring: each judge is called three times and the median taken, then the three are averaged for the leaderboard. Fixed temperature, latest score only, daily rate limit — no gaming the system.',
          '三位 AI 评审（Claude / GPT / Gemini）收到完全相同的评审包（Demo 视频 + 代码 + README / 项目说明）。锚定式绝对评分，每家独立调用 3 次取中位数，三者平均后上榜。温度固定、取最新成绩、每日限频——杜绝刷分。',
        )}
      </p>
    </Window>
  )
