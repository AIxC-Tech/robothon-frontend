...
const COPY: Record<Lang, Copy> = {
  en: {
    ...
    success: (id) => `Registration complete ✓ Your participant ID is ${id}. You'll need it when you submit, so keep it safe. The contest kicks off June 16 (UTC-8, LA time) — follow the guide to open a GitHub PR with this ID attached.`,
    duplicate: (id) => `You're already registered ✓ Your participant ID is ${id}. You'll need it when you submit, so keep it safe. The contest kicks off June 16 (UTC-8, LA time) — follow the guide to open a GitHub PR with this ID attached.`,
    error: 'Sorry, something went wrong saving your registration. Please try again in a moment.',
    registered: (r) =>
      `You're already registered ✓\n· Nickname: ${r.nickname}\n· Agent: ${r.agent}\n· Direction: ${r.direction}\n· Email: ${r.email}\nYour participant ID is ${r.uuid}\nThe contest opens June 16, 2026 (UTC-8, LA time) — submit a GitHub PR with this participant ID.`,
    copyUuid: 'Copy UUID',
  },
  zh: {
    ...
    success: (id) => `报名成功 ✓ 你的参赛 ID 是 ${id}。提交作品时要用，请保存好。6 月 16 日（UTC-8，洛杉矶时间）开赛，届时按指引提交 GitHub PR、附上这个 ID 即可。`,
    duplicate: (id) => `你已经报名过了 ✓ 你的参赛 ID 是 ${id}。提交作品时要用，请保存好。6 月 16 日（UTC-8，洛杉矶时间）开赛，届时按指引提交 GitHub PR、附上这个 ID 即可。`,
    error: '抱歉，保存报名时出错了，请稍后再试。',
    registered: (r) =>
      `你已报名 ✓\n· 昵称：${r.nickname}\n· 工具：${r.agent}\n· 方向：${r.direction}\n· 邮箱：${r.email}\n你的参赛 ID 是：${r.uuid}\n2026 年 6 月 16 日（UTC-8，洛杉矶时间）开赛，提交 GitHub PR、附上参赛 ID`,
    copyUuid: '复制 UUID',
  },
}
